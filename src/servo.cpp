...
namespace {
  ...
  public:
    ...
    virtual int init(jspace::State const & state) {
      ...
      // This can be updated to be parsed from a file
      jspace::Vector goal(jspace::Vector::Zero(3));
      jspace::Vector kp(10*jspace::Vector::Ones(3));
      jspace::Vector kd(jspace::Vector::Ones(3));
      ////////////////////////////////////////////
      jspace::Status status(base_controller->init(state,goal,kp,kd));
      if ( ! status ) {
		       ...
		       jspace::Vector & command)
    {
      ...
      jspace::Status status(base_controller->computeCommand(command));
      if ( ! status ) {
