...
namespace m3_base_ctrl {
  ...
  static void * rt_thread(void * arg)
  {
    ...
    int DOF = 3;
    jspace::State state(DOF, DOF, 6); 
    jspace::Vector command(DOF);
    RTIME tick_period;
    int cb_status;
    ...
    // XXXX Set proper shm.status part to read from
    ...
    for (long long step_cnt(0); 0 == shutdown_request; ++step_cnt) {
      ...
      memcpy(&shm_status, sys->status, sizeof(shm_status));
      rt_sem_signal(status_sem);
      // XXXX Set proper shm_status. part to read from
      for (size_t ii(0); ii < DOF; ++ii) {
	state.position_[ii] = M_PI * shm_status.right_arm.theta[ii] / 180.0;
	state.velocity_[ii] = M_PI * shm_status.right_arm.thetadot[ii] / 180.0;
      }
	...
	continue;
      ...
      // XXXX Set proper shm.cmd part to write to
      for (size_t ii(0); ii < DOF; ++ii) { 
